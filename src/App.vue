<template>
  <div class="grid grid-cols-12 police">
        ...
        <div class="flex flex-col gap-y-2 pt-5 text-lg">
          ...
          >
            <svg
              ...
              stroke-width="1.7"
              stroke="currentColor"
              class="w-6 h-6 group-hover:rotate-180 transition duration-[0.4s]"
            >
              <path
                stroke-linecap="round"
                stroke-linejoin="round"
    ...
    >
    <div class="">
        <div v-if="modalCategory" class=" fixed left-0 top-0 flex justify-center items-center inset-0 z-50 outline-none focus:outline-none bg-no-repeat bg-center bg-cover" id="modal-id">
        ...
        <div v-if="dialogVisible" class=" fixed left-0 top-0 flex justify-center items-center inset-0 z-50 outline-none focus:outline-none bg-no-repeat bg-center bg-cover" id="modal-id">
            <div class="absolute bg-black opacity-80 inset-0 z-0"></div>
            <button class="absolute top-3 right-3 text-white text-xl z-50" @click="dialogVisible = !dialogVisible">
                <i class="pi pi-times"></i>
            </button>
            <div class="w-full  max-w-lg p-5 relative mx-auto my-auto rounded-xl shadow-lg  bg-white ">
                <form @submit.prevent="submitForm" class="flex flex-col space-y-4">
                    <div class="text-xl">Création d'une publication :</div>
                    <input type="text" v-model="postData.title" placeholder="Titre" class="bg-gray-200 rounded-lg py-2 px-4 focus:outline-none" />
                    <textarea type="description" v-model="postData.description" placeholder="Description" class="bg-gray-200 rounded-lg py-2 px-4 focus:outline-none w-full h-40 max-h-40" />
                    <select id="category" v-model="postData.category" required class="text-gray-700 bg-gray-200 rounded-lg py-2 px-4 focus:outline-none">
                      <option class="text-gray-400" value="">Choisir une catégorie</option>
                      <option class="text-gray-700" value="cat1">Catégorie 1</option>
                      <option class="text-gray-700" value="cat2">Catégorie 2</option>
                    </select>
                    <div class="flex justify-end space-x-4">
                        <button class="bg-gray-500 py-2 px-4 rounded-lg font-semibold text-white" @click="dialogVisible = false">Annuler</button>
                        <button type="submit" @click="submitForm" class="bg-green-500 py-2 px-4 rounded-lg font-semibold text-white">Ajouter</button>
                    </div>
                </form>
            </div>
        </div>
