        ...
        <div class="flex items-center">
          ...
          <div>
            <h1 class="text-xl font-bold text-gray-800">
              Profil : {{ getUserById.username }}
            </h1>
            <p v-if="showChangePassword" class="text-sm text-gray-500">
              Email : {{ userStore.user.email }}
            </p>
            <div v-if="showChangePassword">
              <p v-if="userStore.user.role === 'ROLE_USER'" class="text-sm text-gray-500">
                Rôle : User
              </p>
              <p v-if="userStore.user.role === 'ROLE_MOD'" class="text-sm text-gray-500">
                Rôle : Modérateur
              </p>
              <p
              ...
              <p
                v-if="userStore.user.role === 'ROLE_SUPERADMIN'"
                class="text-sm text-gray-500"
              >
        ...
        <div class="mb-4">
          ...
          <form @submit.prevent="updatePassword">
            <div class="grid grid-cols-3">
              ...
              <button
                type="submit"
                class="bg-blue-500 col-span-3 hover:bg-blue-600 text-white font-bold py-2 px-4 rounded-md"
              >
                Changer
              </button>
            </div>
      ...
      <div class="p-6 bg-gray-50">
        ...
        <div class="grid grid-cols-3 gap-4">
          <div v-for="post in postsUser" :key="post.id" class="mb-10">
            <div>
              <img src="/test_blog.jpg" class="bg-auto rounded-3xl h-52" />
              <div class="mt-2">
                <div class="text-2xl text-center font-bold truncate">
                  {{ post.title }}
                </div>
                <div
                  class="mt-2 font-semibold text-gray-500 text-center text-md truncate"
                >
                  {{ post.content }}
                </div>
