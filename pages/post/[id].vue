  ...
  <div class="max-w-5xl mx-auto p-4 relative">
    ...
    <div class="bg-white shadow-md rounded-lg overflow-hidden">
      ...
      <div class="p-6 text-center bg-gray-100">
        <h1 class="text-3xl font-bold text-gray-800">{{ posts.title }}</h1>
        <p class="text-sm text-gray-500 mt-2">Publié le 12 Mars 2024</p>
        <p class="text-sm text-gray-500">par <span class="font-semibold"><a href="#">{{ postItem.author }}</a></span></p>
      </div>
      ...
      <div class="bg-gray-100 p-6 text-center">
        <p class="text-gray-600 text-sm">Catégorie: <span class="font-semibold"><a href="#">{{ postItem.category }}</a></span></p>
        <p class="text-gray-600 text-sm mt-2">Mots-clés: <span class="font-semibold">{{ postItem.tags.join(', ') }}</span></p>
...
export default {
  setup() {
    ...
    const fetchPost = async () => {
      try {
        const response = await axios.get(`http://localhost:8080/api/posts/${route.params.id}`, {
          headers: {
            Authorization: `Bearer ${userStore.user.token}`,
          },
        });
        posts.value = [response.data]; // Assign the fetched post to the posts array
      } catch (error) {
